...
// sign in
authRouter.use('/signin', (req, res, next) => ensureUnauthenticated(req, res, next))

authRouter.route('/signin')
        .get((req, res) => {
            res.status(200).json({message: 'Welcome! Please sign in here'})
        })
        .post((req, res, next) => {
...
// sign up
authRouter.use('/signup', (req, res, next) => ensureUnauthenticated(req, res, next))

authRouter.get('/signup', (req, res) => {
    res.status(200).json({message: 'Welcome! Please sign up here'})
})
...
authRouter.post('/signup', (req, res, next) => {
    User.findOne({email: req.body.email}, (err, user) => {
        if (err) {
            ...
            bcrypt.hash(req.body.password, saltRounds, function(error, hash) { // hash the inputted password
                if (error) {
                    return next(error)
                }
                let plainVerifyId = shortid.generate()
                User.create({...req.body, password: hash, verification: {verified: false, verifyId: bcrypt.hashSync(plainVerifyId, saltRounds)}, wantToReset: false }, e => {
                  if (e) {
    ...
    User.findOne({email: req.body.email}, (err, user) => {
        ...
        if (!user) {
            return res.status(400).json({message: 'An error occured during signup: user not found'})
        }
        // send verification ID
        sendVerifyIdOrResetCode(user, res.locals.plainVerifyId, 'verification ID')
        res.redirect('/api/v1/auth/verify?email=' + user.email) // redirect to verify
    })
})

// verification
authRouter.use('/verify', (req, res, next) => ensureAuthenticated(req, res, next))

authRouter.get('/verify', (req, res) => {
    return res.status(200).json({message: 'Signup successful. Please enter the verification ID sent to ' + req.query.email})
})
...
authRouter.use('/resend-verifyId', (req, res, next) => ensureAuthenticated(req, res, next))
...
// logout
authRouter.get('/signout', (req, res, next) => ensureAuthenticated(req, res, next), (req, res) => {
    req.logout()
...
authRouter.post('/reset-password', (req, res) => {
    User.findOne({ email: req.body.email }, (err, user) => {
        ...
        if (!user || user.resetCodeSent) { // no user or reset code already sent
            return res.status(400).json({message: 'Invalid email'})
        }
        // generate new verification ID
        ...
        return res.redirect('/api/v1/auth/confirm-reset-code?id=' + user._id + '&resend=false')
    })
})

authRouter.get('/confirm-reset-code', (req, res) => {
    if (req.query.resend === 'false') {
        return res.status(200).json({message: 'Please enter the reset code sent to you'})
...
authRouter.get('/new-password', (req, res) => {
    User.findOne({ _id: req.query.id }, (err, user) => {
        ...
        if (!user || user.resetCode || !user.wantToReset) { // still have reset code or incorrect id or dont want to reset
            return res.status(400).json({message: 'Invalid path'})
        }
